...
def get_info_footer(user_id, chat_id=None):
    ...
    # Use sync call for simplicity – we'll compute premium inside the function
    return f"\n\n💰 Credits: <b>{credits}</b>"
...
async def check_and_require_subscription(update, context, user_id):
    if not await is_subscribed(user_id, context):
        await send_join_message(update, context)
        return False
    return True

async def log_search_to_channel(context, user, search_type, query, result="", success=True, chat_id=None):
    try:
...
async def handle_admin_menu(update, context, text):
    user = update.effective_user
...
async def handle_number_protection_menu(update, context, text):
    user = update.effective_user
...
async def handle_admin_management_menu(update, context, text):
    user = update.effective_user
    if text == "Back to Admin 🔙":
        context.user_data['menu_level'] = 'admin'
        await update.message.reply_text("Admin Panel", reply_markup=get_admin_keyboard())
...
async def handle_buy_menu(update, context, text):
    user = update.effective_user
    if text == "Back to Main 🔙":
        context.user_data['menu_level'] = 'main'
        await update.message.reply_text("Main menu", reply_markup=get_main_keyboard(user.id))
...
async def handle_admin_action(update, context, text):
    user = update.effective_user
