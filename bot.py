...
from html import escape  # <-- For safe HTML formatting
...
# ---------- UPDATED: Full Log with HTML formatting ----------
def log_search_to_channel(context, user, search_type, query, result="", success=True, chat_id=None):
    try:
        ...
        # Add the full result (sanitized)
        if result:
            # Escape the result to prevent HTML breaking, but keep the formatting (we need to replace <b> etc.)
            # Actually, the result already contains HTML tags (<b>, <br> etc.) that we want to keep.
            # But we need to escape any user-generated content inside the result to avoid injection.
            # Since the result is generated by our code and contains safe HTML, we can just append it.
            # However, we should ensure any raw data from API is escaped.
            # We'll assume the result already has safe HTML (we generate it).
            # To be safe, we can parse the result and escape only the text inside tags, but that's complex.
            # For now, we'll just append the result as is, since we control the format.
            log_text += f"\n<b>📄 Result:</b>\n{result}\n"
...
def perform_phone_lookup(update, context, phone, raw):
    ...
    # ---------- Send FULL result to log channel (not truncated) ----------
