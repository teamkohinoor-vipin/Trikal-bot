...
async def can_use_daily_free(user_id):
    daily_count, _ = get_daily_data(user_id)
    return daily_count < get_daily_free_limit()

async def deduct_credits(user_id, chat_id=None):
...
async def check_membership(user_id, channel_id, context: CallbackContext):
    try:
...
async def is_subscribed(user_id, context: CallbackContext):
...
async def send_join_message(update: Update, context: CallbackContext):
    ...
    await msg.reply_text("❌ <b>You must join both channels to use this bot!</b>\n\nJoin and click Verify.",
                         reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)

async def check_and_require_subscription(update: Update, context: CallbackContext, user_id):
    if not await is_subscribed(user_id, context):
        await send_join_message(update, context)
        return False
    return True

async def log_search_to_channel(context: CallbackContext, user, search_type, query, result="", success=True, chat_id=None):
    try:
        ...
        await context.bot.send_message(chat_id=LOG_CHANNEL_ID, text=msg, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    except Exception as e:
        logger.error(f"Log error: {e}")

async def broadcast_message(context: CallbackContext, message):
    success = 0
    fail = 0
...
async def start(update: Update, context: CallbackContext):
    ...
    if is_maintenance_mode_active() and not is_admin(user.id):
        await update.message.reply_text("⚠️ Maintenance mode active. Try later.")
        return
    if is_banned(user.id):
        return
    if chat.type != 'private' and chat.id != OFFICIAL_GROUP_ID:
        await update.message.reply_text("❌ This bot works only in private chat or official group.")
        return
    ...
    await update.message.reply_text(text, reply_markup=get_main_keyboard(user.id), parse_mode=ParseMode.HTML)
...
async def help_command(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=get_main_keyboard(user.id))
...
async def phone_command(update: Update, context: CallbackContext):
    ...
    if not normalized:
        ...
        return
    await perform_phone_lookup(update, context, normalized, raw)

async def perform_phone_lookup(update: Update, context: CallbackContext, phone, raw):
    user = update.effective_user
    chat = update.effective_chat
    if is_number_protected(phone):
        ...
        for p in protected:
            if p["_id"] == phone:
                await update.message.reply_text(p.get("message", "❌ No data."), parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_main')]]))
                await log_search_to_channel(context, user, "Phone", raw, "Protected", False, chat.id)
                return
    use_daily = False
        ...
        if not (is_global_free_mode_active() or is_free_mode_active() or is_admin(user.id) or await is_premium(user.id)):
            if await can_use_daily_free(user.id):
                ...
                if not await deduct_credits(user.id, chat.id):
                    await update.message.reply_text("❌ Insufficient credits. Use referrals or daily free.")
                    return
    ...
    records = await fetch_phone_info(phone)
    if not records:
        footer = get_info_footer(user.id, chat.id)
        await msg.edit_text(f"❌ No data found for {phone}.\n{footer}", parse_mode=ParseMode.HTML)
        await log_search_to_channel(context, user, "Phone", raw, "No data", False, chat.id)
        return
    result = f"🔍 <b>Results for {phone}</b>\n\n"
    for i, rec in enumerate(records[:10], 1):
        result += f"✅ <b>Result {i}:</b>\n"
        for k, v in rec.items():
            if v and str(v).lower() not in ['', 'n/a', 'null']:
                if k == 'address':
                    v = format_address(v)
                result += f"<b>{k.replace('_',' ').title()}:</b> {v}\n"
        result += "\n━━━━━━━━━━━━━━━━\n\n"
    result += "\n💞<b>Developer: @ll_VIPIN_ll</b>"
    if use_daily:
        daily, _ = get_daily_data(user.id)
        limit = get_daily_free_limit()
        footer = f"\n\n🎁 Daily free search used: {daily}/{limit} ({(limit-daily)} left)"
    else:
        footer = get_info_footer(user.id, chat.id)
    final = result + footer
    ...
    await msg.edit_text(final, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
    ...
    await log_search_to_channel(context, user, "Phone", raw, result[:300], True, chat.id)
...
async def redeem_command(update: Update, context: CallbackContext):
    ...
    if not context.args:
        ...
        await update.message.reply_text("🎁 Send your redeem code:", reply_markup=get_main_keyboard(user.id))
        return
    code = context.args[0].strip().upper()
    credits = use_redeem_code(code, user.id)
    if credits is None:
        await update.message.reply_text("❌ Invalid, used, or on cooldown.", reply_markup=get_main_keyboard(user.id))
    else:
        await update.message.reply_text(f"✅ {credits} credits added!", reply_markup=get_main_keyboard(user.id))
...
async def button_handler(update: Update, context: CallbackContext):
    ...
    if data == 'back_to_main':
        ...
        if await is_subscribed(user.id, context):
            ...
            await query.edit_message_text("❌ Not subscribed to both channels.")
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if is_admin(user.id) and context.user_data.get('admin_action'):
        await handle_admin_action(update, context, text)
        return

    if context.user_data.get('state') == 'awaiting_redeem':
        context.user_data['state'] = None
        credits = use_redeem_code(text.upper(), user.id)
        await update.message.reply_text(f"✅ {credits} credits added!" if credits else "❌ Invalid code.", reply_markup=get_main_keyboard(user.id))
        return

    norm = normalize_phone_number(text)
    if norm:
        await perform_phone_lookup(update, context, norm, text)
        return
    ...
    if menu == 'main':
        ...
        await handle_buy_menu(update, context, text)
    elif menu == 'admin_number_protection':
        await handle_number_protection_menu(update, context, text)
    elif menu == 'admin_management':
        await handle_admin_management_menu(update, context, text)
    else:
        context.user_data['menu_level'] = 'main'
        await update.message.reply_text("Main menu", reply_markup=get_main_keyboard(user.id))

# ---------- Menu Handlers ----------
async def handle_main_menu(update: Update, context: CallbackContext, text):
    user = update.effective_user
    if text == "India Number 🇮🇳":
        context.user_data['state'] = 'awaiting_search'
        await update.message.reply_text("🔍 Send 10-digit Indian mobile number:", reply_markup=get_main_keyboard(user.id))
        ...
        await update.message.reply_text(f"👨‍💻 Contact @{SUPPORT_USERNAME}", parse_mode=ParseMode.HTML, reply_markup=get_main_keyboard(user.id))
    elif text == "Official Group 🚀":
        await update.message.reply_text(f"🚀 Official Group: {OFFICIAL_GROUP_LINK}\n\nJoin for unlimited free searches!", parse_mode=ParseMode.HTML, reply_markup=get_main_keyboard(user.id))
    elif text == "Privacy Policy 🔒":
        await update.message.reply_text("🔒 We do not store any personal data. Only credits and referral counts are kept.", parse_mode=ParseMode.HTML, reply_markup=get_main_keyboard(user.id))
...
async def handle_admin_menu(update: Update, context: CallbackContext, text):
    user = update.effective_user
    if text == "Back to Main 🔙":
        ...
        await update.message.reply_text(f"Current auto-delete: {get_auto_delete_time()} seconds.\nSend new time (0 = off):", reply_markup=get_admin_keyboard())
    elif text == "Maintenance Mode ⚠️":
        new = not is_maintenance_mode_active()
        set_maintenance_mode(new)
        await update.message.reply_text(f"⚠️ Maintenance mode is {'ON' if new else 'OFF'}.", reply_markup=get_admin_keyboard())
        log_user_action(user.id, "Toggled Maintenance Mode", str(new))
    elif text == "Add Credits ➕":
        context.user_data['admin_action'] = 'add_credits'
        await update.message.reply_text("💰 Send: user_id credits", reply_markup=get_admin_keyboard())
        ...
        await update.message.reply_text("⭐ Send: user_id [days]", reply_markup=get_admin_keyboard())
    elif text == "Remove Premium ⭐➖":
        context.user_data['admin_action'] = 'remove_premium'
        await update.message.reply_text("⭐ Send user_id", reply_markup=get_admin_keyboard())
    elif text == "Add Credits to All 💰👥":
        context.user_data['admin_action'] = 'add_credits_all'
        await update.message.reply_text("💰 Enter number of credits for every user:", reply_markup=get_admin_keyboard())
    elif text == "User History 📝":
        await update.message.reply_text("Use /history <user_id>", reply_markup=get_admin_keyboard())
    elif text == "Broadcast 📢":
        context.user_data['admin_action'] = 'broadcast'
        await update.message.reply_text("📢 Send the broadcast message:", reply_markup=get_admin_keyboard())
        ...
        set_free_mode(new)
        await update.message.reply_text(f"🎯 Group Free Mode is {'ON' if new else 'OFF'}.", reply_markup=get_admin_keyboard())
    elif text == "Toggle Global Free 🌍":
        new = not is_global_free_mode_active()
        set_global_free_mode(new)
        await update.message.reply_text(f"🌍 Global Free Mode is {'ON' if new else 'OFF'}. Notifying users...", reply_markup=get_admin_keyboard())
        for u in users.find():
            try:
                await context.bot.send_message(chat_id=u["_id"], text=f"🌍 Global free mode {'activated' if new else 'deactivated'}.", parse_mode=ParseMode.HTML)
            except:
                pass
        ...
        await update.message.reply_text("🔢 Enter new daily free limit:", reply_markup=get_admin_keyboard())
...
async def handle_number_protection_menu(update: Update, context: CallbackContext, text):
    user = update.effective_user
    if text == "Back to Admin 🔙":
        ...
        await update.message.reply_text("🛡️ Send number and optional message:", reply_markup=get_number_protection_keyboard())
        ...
        protected = get_all_protected_numbers()
        if not protected:
            await update.message.reply_text("No protected numbers.", reply_markup=get_number_protection_keyboard())
        else:
            msg = "🛡️ Protected Numbers:\n"
            for p in protected[:20]:
                msg += f"📱 <code>{p['_id']}</code> - {p.get('message', 'No message')}\n"
            await update.message.reply_text(msg, parse_mode=ParseMode.HTML, reply_markup=get_number_protection_keyboard())

async def handle_admin_management_menu(update: Update, context: CallbackContext, text):
    user = update.effective_user
    if text == "Back to Admin 🔙":
        context.user_data['menu_level'] = 'admin'
        await update.message.reply_text("Admin Panel", reply_markup=get_admin_keyboard())
        ...
        await update.message.reply_text(msg, parse_mode=ParseMode.HTML, reply_markup=get_admin_management_keyboard())

async def handle_buy_menu(update: Update, context: CallbackContext, text):
    user = update.effective_user
    if text == "Back to Main 🔙":
        context.user_data['menu_level'] = 'main'
        await update.message.reply_text("Main menu", reply_markup=get_main_keyboard(user.id))
        ...
        await update.message.reply_text("💰 Credit Packages:\n10 Credits - ₹15\n27 Credits - ₹35\n55 Credits - ₹65\n115 Credits - ₹110\n250 Credits - ₹200\n\nContact @KHRsupportBot", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Contact Support", url=f"https://t.me/{SUPPORT_USERNAME}")]]))

# ---------- Admin Action Handler ----------
async def handle_admin_action(update: Update, context: CallbackContext, text):
    user = update.effective_user
    ...
    if action == 'add_credits':
        parts = text.split()
        if len(parts) != 2:
            await update.message.reply_text("❌ Usage: user_id credits", reply_markup=get_admin_keyboard())
        else:
            try:
                uid = int(parts[0])
                credits = int(parts[1])
                update_credits(uid, credits)
                await context.bot.send_message(chat_id=uid, text=f"💰 Admin added {credits} credits to your account.", parse_mode=ParseMode.HTML)
                await update.message.reply_text(f"✅ Added {credits} credits to {uid}", reply_markup=get_admin_keyboard())
        ...
        if len(parts) != 2:
            ...
            try:
                ...
                credits = int(parts[1])
                update_credits(uid, -credits)
                await context.bot.send_message(chat_id=uid, text=f"💰 Admin removed {credits} credits from your account.", parse_mode=ParseMode.HTML)
                await update.message.reply_text(f"✅ Removed {credits} credits from {uid}", reply_markup=get_admin_keyboard())
        ...
        try:
            ...
            await context.bot.send_message(chat_id=uid, text=f"⭐ Admin granted you premium {'for '+str(days)+' days' if days else 'permanently'}!", parse_mode=ParseMode.HTML)
            await update.message.reply_text(f"✅ Premium added to {uid}", reply_markup=get_admin_keyboard())
            log_user_action(user.id, "Added Premium", f"To: {uid}, Days: {days}")
        except:
            await update.message.reply_text("❌ Usage: user_id [days]", reply_markup=get_admin_keyboard())
        context.user_data['admin_action'] = None
    elif action == 'remove_premium':
        try:
            uid = int(text)
            remove_premium(uid)
            await context.bot.send_message(chat_id=uid, text="⚠️ Your premium access has been removed by admin.", parse_mode=ParseMode.HTML)
            await update.message.reply_text(f"✅ Premium removed from {uid}", reply_markup=get_admin_keyboard())
        ...
        try:
            ...
            for u in users.find():
                ...
                try:
                    await context.bot.send_message(chat_id=u["_id"], text=f"💰 Admin gave {credits} credits to all users!", parse_mode=ParseMode.HTML)
                except:
                    pass
                count += 1
            await update.message.reply_text(f"✅ Added {credits} credits to {count} users.", reply_markup=get_admin_keyboard())
        ...
        success, fail = await broadcast_message(context, text)
        await update.message.reply_text(f"📢 Broadcast: ✅ {success} success, ❌ {fail} failed", reply_markup=get_admin_keyboard())
        ...
        try:
            uid = int(text)
            ban_user(uid)
            await context.bot.send_message(chat_id=uid, text="🚫 You have been blocked from using this bot.", parse_mode=ParseMode.HTML)
            await update.message.reply_text(f"✅ Blocked {uid}", reply_markup=get_admin_keyboard())
        ...
        try:
            uid = int(text)
            unban_user(uid)
            await context.bot.send_message(chat_id=uid, text="✅ You have been unblocked. You can use the bot again.", parse_mode=ParseMode.HTML)
            await update.message.reply_text(f"✅ Unblocked {uid}", reply_markup=get_admin_keyboard())
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Daily free limit set to {limit}.", reply_markup=get_admin_keyboard())
            log_user_action(user.id, "Set Daily Limit", str(limit))
        except:
            await update.message.reply_text("❌ Enter a number.", reply_markup=get_admin_keyboard())
        context.user_data['admin_action'] = None
    elif action == 'set_auto_delete':
        try:
            sec = int(text)
            set_auto_delete_time(sec)
            await update.message.reply_text(f"✅ Auto-delete time set to {sec} seconds.", reply_markup=get_admin_keyboard())
        except:
            await update.message.reply_text("❌ Invalid number.", reply_markup=get_admin_keyboard())
        context.user_data['admin_action'] = None
        ...
        if not parts:
            await update.message.reply_text("❌ Usage: number [message]", reply_markup=get_number_protection_keyboard())
        else:
            num = parts[0]
            msg = parts[1] if len(parts) > 1 else None
            if protect_number(num, user.id, msg):
                await update.message.reply_text(f"✅ Number {num} protected.", reply_markup=get_number_protection_keyboard())
                log_user_action(user.id, "Protected Number", num)
            else:
                await update.message.reply_text("❌ Already protected.", reply_markup=get_number_protection_keyboard())
        context.user_data['admin_action'] = None
    elif action == 'unprotect_number':
        num = text.strip()
        if unprotect_number(num):
            await update.message.reply_text(f"✅ Number {num} unprotected.", reply_markup=get_number_protection_keyboard())
            log_user_action(user.id, "Unprotected Number", num)
        else:
            await update.message.reply_text("❌ Not protected.", reply_markup=get_number_protection_keyboard())
        context.user_data['admin_action'] = None
    elif action == 'add_admin':
        try:
            uid = int(text)
            add_admin(uid)
            await context.bot.send_message(chat_id=uid, text="👑 You have been promoted to admin! Use /admin to access panel.", parse_mode=ParseMode.HTML)
            await update.message.reply_text(f"✅ Admin {uid} added.", reply_markup=get_admin_management_keyboard())
            log_user_action(user.id, "Added Admin", str(uid))
        except:
            await update.message.reply_text("❌ Invalid ID.", reply_markup=get_admin_management_keyboard())
        context.user_data['admin_action'] = None
    elif action == 'remove_admin':
        try:
            uid = int(text)
            if uid in ADMIN_IDS:
                await update.message.reply_text("❌ Cannot remove owner.", reply_markup=get_admin_management_keyboard())
            else:
                remove_admin(uid)
                await context.bot.send_message(chat_id=uid, text="⚠️ Your admin privileges have been removed.", parse_mode=ParseMode.HTML)
                await update.message.reply_text(f"✅ Admin {uid} removed.", reply_markup=get_admin_management_keyboard())
...
async def history_command(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if not hist:
            await update.message.reply_text(f"No history for {uid}.")
            return
...
async def protect_command(update: Update, context: CallbackContext):
    ...
    if len(context.args) < 1:
        await update.message.reply_text("Usage: /protect <number> [message]")
        return
    num = context.args[0]
...
async def addadmin_command(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=uid, text="👑 You are now an admin.", parse_mode=ParseMode.HTML)
    except:
