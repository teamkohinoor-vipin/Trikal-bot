...
def perform_phone_lookup(update, context, phone, raw):
    ...
    records = fetch_phone_info(phone)  # note: fetch_phone_info is async? No, it's sync because requests.get is blocking. We'll redefine it as sync.
    if not records:
        footer = get_info_footer(user.id, chat.id)
        msg.edit_text(f"❌ No data found for {phone}.\n{footer}", parse_mode=ParseMode.HTML)
    ...
    msg.edit_text(final, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
